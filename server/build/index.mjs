...
var worker = {
  async fetch(req, env) {
    ...
    const User2 = client.db("test").collection("users");
    const Profile = client.db("test").collection("profiles");
    const RecallUser = client.db("test").collection("recallusers");
    router.get("/:userId", async (req2, res) => {
      const user2 = await User2.findOne({
        _id: new ObjectId2(req2.params.userId)
      });
      delete user2?.password;
      const body = JSON.stringify({ status: true, user: user2 });
      const headers = {
        ...corsHeaders,
        "Content-type": "application/json"
      };
    ...
    router.post("/api/users/getRecallsByUser", async (req2, res) => {
      const content = await req2.json();
      const recalls = await Profile.find({ userId: content.userId });
      return createResponse({ recalls });
    });
    router.post("/login", async (req2, res) => {
      console.log("logging in =>");
      const content = await req2.json();
      const user2 = await User2.findOne(content);
      if (user2) {
        return createResponse({ status: true, user: user2 });
      } else {
        return createResponse({ message: "Wrong email or password" });
      }
    });
    router.post("/signup", async (req2, res) => {
      const content = await req2.json();
      const user2 = await User2.findOne({
        email: content.email
      });
      if (user2) {
        ...
        const u = await User2.insertOne(content);
        return createResponse({ status: true, userId: u.insertedId });
      }
    });
    router.get("/api/getRecalls", async (req2, res) => {
      const recalls = await Profile.find({});
      return createResponse({ recalls });
    });
    router.get("/api/users", async (req2, res) => {
      const recalls = await Profile.find({});
      return createResponse({ recalls });
    });
    ...
    router.post("/sort", (req2, res) => {
      User2.find().then((users) => {
        switch (req2.body.attribute) {
          case "firstname":
            return [...users].sort((a, b) => a.firstname > b.firstname ? 1 : a.firstname === b.firstname ? a.lastname > b.lastname ? 1 : a.lastname === b.lastname ? a.age > b.age ? 1 : a.age === b.age ? a.sex > b.sex ? 1 : -1 : -1 : -1 : -1);
          case "lastname":
            return [...users].sort((a, b) => a.lastname > b.lastname ? 1 : a.lastname === b.lastname ? a.firstname > b.firstname ? 1 : a.firstname === b.firstname ? a.age > b.age ? 1 : a.age === b.age ? a.sex > b.sex ? 1 : -1 : -1 : -1 : -1);
          case "sex":
            return [...users].sort((a, b) => a.sex > b.sex ? 1 : a.sex === b.sex ? a.firstname > b.firstname ? 1 : a.firstname === b.firstname ? a.lastname > b.lastname ? 1 : a.lastname === b.lastname ? a.age > b.age ? 1 : -1 : -1 : -1 : -1);
          case "age":
            return [...users].sort((a, b) => a.age > b.age ? 1 : a.age === b.age ? a.firstname > b.firstname ? 1 : a.firstname === b.firstname ? a.lastname > b.lastname ? 1 : a.lastname === b.lastname ? a.sex > b.sex ? 1 : -1 : -1 : -1 : -1);
          default:
            return [...users];
        }
      }).then((users) => res.status(200).json(users)).catch((err) => res.status(500).json({ error: "Failed to sort" }));
    });
    router.post("/api/users/recall", async (req2, res) => {
      const content = await req2.json();
    ...
    router.put("/:userId", (req2, res) => {
    });
    router.delete("/:userId", (req2, res) => {
    });
    router.post("/api/users/createprofile", async (req2, res) => {
      const content = await req2.json();
    ...
    router.put("/updateProfile/:userId", (req2, res) => {
    });
    router.post("/api/users/findprofile/:query", async (req2, res) => {
      const query = req2.params.query;
      var results = await Profile.find({
        state: query
      });
      return createResponse(results);
    });
    router.get("/api/users/findsigner", async (req2, res) => {
      const profile_id = req2.url.split("?id=")[1];
      var r = await RecallUser.find({
        profile_id
      });
      return createResponse(r);
    });
    router.get("/api/users/profile", async (req2, res) => {
      const profile_id = req2.url.split("?id=")[1];
    ...
    router.post("/api/users/findemailsigner/:query", async (req2, res) => {
      const query = req2.params.query;
    ...
    router.post("/api/users/findphonesigner/:query", async (req2, res) => {
      const query = req2.params.query;
